...
export default function CartScreen() {
  ...
  const { cart, removeItem, updateQuantity, clearCart, total, loaded } = useCart();
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={clearCart} disabled={isEmpty}>
          <MaterialCommunityIcons name="delete-sweep" size={24} color={isEmpty ? '#ccc' : '#ef4444'} />
        </TouchableOpacity>
      </View>

      {isEmpty ? (
        <View style={styles.emptyContainer}>
          <MaterialCommunityIcons name="shopping" size={64} color={colors.textSecondary} />
          <Text style={styles.emptyText}>Votre panier est vide</Text>
          <TouchableOpacity style={styles.continueBtn} onPress={() => router.push('/client')}>
        ...
        <>
          ...
          <View style={styles.summary}>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Sous-total ({cart.reduce((sum, item) => sum + item.quantity, 0)} articles)</Text>
...
const styles = StyleSheet.create({
  ...
  continueBtn: { backgroundColor: colors.primary, paddingVertical: 12, paddingHorizontal: 24, borderRadius: 8 },
  ...
  qtyControl: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#f3f4f6', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 4, alignSelf: 'flex-start' },
  qtyBtn: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  qtyBtnText: { fontSize: 14, fontWeight: '700', color: colors.textPrimary },
  qty: { fontSize: 12, fontWeight: '600', width: 20, textAlign: 'center' },
  summary: { backgroundColor: '#fff', borderTopWidth: 1, borderColor: '#e5e7eb', paddingHorizontal: 16, paddingVertical: 12 },
